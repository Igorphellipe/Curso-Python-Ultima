...
def validar_medida(medida):
    try: medida_validada = int(medida)
...
def ler_peso_objeto():
    ...
    while not lido_sucesso:
        peso_lido = input('Digito o peso do objeto (em Kg): ')
        tente: peso = float(peso_lido)
        if peso > 30:
            print('Não é aceito objetos tão pesados')
            print('Por favor entre com o peso novamente')
        continue

    try: lido_sucesso = Verdadeiro
    except:
...
def ler_rota():
    ...
    while not lido_sucesso:
        ...
        rotas = input('Digite a rota desejada (utilize como siglas): ')
        rota.append(rotas)
